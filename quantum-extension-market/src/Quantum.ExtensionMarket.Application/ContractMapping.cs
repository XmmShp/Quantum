...
{
    public static UserSummary ToSummary(this MarketUser user)
        => new()
        {
            UserId = Format(user.Id),
            Username = user.Username,
            Email = user.Email,
            Roles = (MarketUserRoles)(int)user.Roles,
    ...
    public static string Format(MarketUserId id) => ((long)id).ToString(CultureInfo.InvariantCulture);
