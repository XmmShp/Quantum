...
public sealed class MarketUser
{
    ...
    public MarketUserId Id { get; private set; }
    public string Username { get; private set; } = string.Empty;
    public string Email { get; private set; } = string.Empty;
    public string PasswordHash { get; private set; } = string.Empty;
    public MarketUserRole Roles { get; private set; }
    public DateTime CreatedAtUtc { get; private set; }
    public DateTime LastLoginAtUtc { get; private set; }

    public static MarketUser Create(
        string username,
        string email,
        string passwordHash,
        MarketUserRole roles = MarketUserRole.User | MarketUserRole.Developer,
        IIdGenerator? idGenerator = null,
        TimeProvider? timeProvider = null)
        => new(
            MarketUserId.New(idGenerator.OrDefault()),
            username,
            email,
            passwordHash,
            roles,
    ...
    public void SetRoles(MarketUserRole roles) => Roles = ValidateRoles(roles);

    public void RecordLogin(TimeProvider? timeProvider = null)
        => LastLoginAtUtc = timeProvider.OrDefault().GetUtcNow().UtcDateTime;

    public bool HasAnyRole(MarketUserRole roles) => (Roles & roles) != 0;
    ...
    private static MarketUserRole ValidateRoles(MarketUserRole roles)
    {
        const MarketUserRole all = MarketUserRole.User | MarketUserRole.Developer |
                                   MarketUserRole.Reviewer | MarketUserRole.Admin;
        if (roles == MarketUserRole.None || (roles & ~all) != 0)
        {
