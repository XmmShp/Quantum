...
- `SDK/Quantum.Plugin.Abstractions` 是宿主与插件共享的稳定 ABI，独立于 Host 和 Infrastructure；程序集名和命名空间是插件兼容性边界。
...
1. `System.*`、`Microsoft.*`、`NOF.*`、`Quantum.Plugin.Abstractions` 和 `Quantum.Contract` 委托给默认上下文。
2. 其余托管依赖优先由插件目录解析。
